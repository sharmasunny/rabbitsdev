...
/*
*	@this query will select record from collections table where collection handle match 
...
/*
*	@this query will select all record from store table 
...
while($stores_record = $stores_records->fetch_assoc()) {
	while($products_record = $products_records->fetch_assoc()) {
		...
		if (is_array($productstorelistarr) || is_object($productstorelistarr))
		{
		    ...
		    foreach($productstorelistarr as $storeid) {
		    	/*
				*	@this query will select all record from store table where store id match 
				*	@field store_id
				*	@field store_is_deleted
				*/
	...
	if($pointLocation->pointInPolygon($latitude_y.",".$longitude_x, $polygon)=='inside')
	{
		...
		/*
		*	@this query will select all record from distance table where zone_one match and distance 
		*	@field distance
		*	@field zone_one
		*/
		$zonequery = "SELECT * FROM distance where distance != '' and zone_one = '".$zoneitem."' ORDER BY CAST( distance AS DECIMAL( 10, 5 ) ) ASC";
				...
				foreach($mainarr as $key => $value) {
					...
					if($store_id == $storeid )
					{
						if(in_array($storeid,$uniquecollectionstore)){
							...
							/*
							*	@this query will select all record from stores table where store_id match 
							*	@field store_id
							*	@field store_is_deleted
							*	@field store_postalcode
							*/
							...
							while($st_record = $st_records->fetch_assoc()) 	
								/*
								*	@this query will select all record from zones table where zone_id match 
								*	@field zone_id
								*	@field store_is_deleted
								*	@field store_postalcode
								*/													{
							...
							/*
							*	@this query will select all record from distance table where zone_one and zone_two match 
							*	@field zone_one
							*	@field zone_two
							*/	
							$query = "SELECT * FROM `distance` where zone_one = '".$zoneitem."' and zone_two ='".$zonenamerecord['zone_name']."' ORDER BY CAST( distance AS DECIMAL( 10, 5 ) ) ASC ";
						
							$distance_records = $conn->query($query);
							$distance_record = $distance_records->fetch_assoc();
							/*
							*	@this query will select minmium  price regular from  price
							*	@field zone_from
							*/	
...
while($stores_record = $stores_records->fetch_assoc()) {
	...
	$latitude_y   = $stores_record['store_longitude'];
	 foreach($fzone_coor as $indx=>$fzone_coordinate) {					
		...
		/*
		*	@this query will select all record from  distance table
		*	@field distance
		*	@field zone_one
		*/
		$zonequery = "SELECT * FROM distance where distance != '' and zone_one = '".$zoneitem."' ORDER BY CAST( distance AS DECIMAL( 10, 5 ) ) ASC";
				...
				foreach($mainarr as $key => $value) {
					...
					if(!in_array($storeid,$uniquestore)){
							...
							*/	
							...
							/*
							*	@this query will select price regular from  price table 
							*	@field zone_from
							*/
							$query1 = "SELECT MIN(price_regular) as price FROM `zone_detail` where zone_from = '".$st_record['store_zone_id']."'";
