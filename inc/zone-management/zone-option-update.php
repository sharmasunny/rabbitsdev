...
if($info == "delall") {
			...
			if (count($id) > 0) {
				if(!empty($delete_id)) {
					foreach ($delete_id as $id_d) {
						/*
						*	@this query will delete selected record from zone_detail table 
						*/
	...
	if ( $info=="del" ) {
		...
		if ( !empty( $delid ) ) {
			/*
			*	@this query will delete record from zone_detail table 
			*/
...
if($info="edit") {
		...
		if ( !empty( $editlid ) ) { 
			/*
			*	@this query will select all record from zone_detail table 
			*/
			...
			$selected_product_records = $conn->query($selected_store_sql);
				...
				<div class="wrap"> 
					...
					<br/>
					 ...
					 <form role="form" action="zone-option-edit.php" method="post" id="editzoneoptionform">
