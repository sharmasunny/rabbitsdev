			...
			if (count($id) > 0) {
				if(!empty($delete_id)) {
					foreach ($delete_id as $id_d) {
						/*
						*	@this query will delete record from zone details table 
						*/
...
if($info="edit") {
		...
		if ( !empty( $editlid ) ) { 
			/*
			*	@this query will select all record from zone details table 
			*/
			...
			$selected_product_records = $conn->query($selected_store_sql);
				...
				<div class="wrap"> 
					...
					<br/>
					 ...
					 <form role="form" action="medium-option-edit.php" method="post" id="editzoneoptionform">
					...
					*/
					...
					if ( $all_zone_records->num_rows > 0 ) {
						while($all_zone_record = $all_zone_records->fetch_assoc()) {
							...
							{
								echo "<option selected='selected' id=".$all_zone_record['zone_id']." value='".htmlspecialchars($all_zone_record['zone_name'])."'>".$all_zone_record['zone_name']."</option>";
							}
							else{
							 echo "<option id=".$all_zone_record['zone_id']." value='".htmlspecialchars($all_zone_record['zone_name'])."'>".$all_zone_record['zone_name']."</option>";
							}
						}	
					}
				  	?>
					...
					$all_zone_records 	  = $conn->query($all_zones_sql);
				  	if ( $all_zone_records->num_rows > 0 ) {
						...
						while($all_zone_record = $all_zone_records->fetch_assoc()) {
							...
							{
								echo "<option selected='selected' id=".$all_zone_record['zone_id']." value='".$all_zone_record['zone_id']."'>".$all_zone_record['zone_name']."</option>";
							}
							else{
