...
if(!empty($_POST)) {
	...
	{
		echo "<script>alert('Error while Updating Record. Please select zone!!');</script>";
	...
	*/
	...
	if ($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			if($row['total'] > 0)
			{
			echo "<script>alert('Error while Updating Record. Zone option already exists!!');</script>";
			 echo "<script>parent.jQuery.fancybox.close();</script>";
			echo "<script>window.location = '".HOME_URL."'</script>";
			}
		}
