...
while($zonecoorinate = $zonecoorinates->fetch_assoc()){
	...
	$zone_coors[$count]['zone_central_zipcode'] = $zonecoorinate['zone_central_zipcode'];
	$zone_coors[$count]['zone_centralzipcode_latitude'] = $zonecoorinate['zone_centralzipcode_latitude'];
	$zone_coors[$count]['zone_centralzipcode_longitude'] = $zonecoorinate['zone_centralzipcode_longitude'];
...
foreach($zone_coors as $key => $zone_coor) {
	$fzone_coor[$key]['zone_central_zipcode'] = $zone_coor['zone_central_zipcode'];
	$fzone_coor[$key]['zone_centralzipcode_latitude'] = $zone_coor['zone_centralzipcode_latitude'];
	$fzone_coor[$key]['zone_centralzipcode_longitude'] = $zone_coor['zone_centralzipcode_longitude'];
	$a = 0;
...
foreach($fzone_coor as $indx=>$fzone_coordinate) {
	...
	if(!empty($_REQUEST)) {
		if($pickup==$fzone_coordinate['zone_central_zipcode']){
			$longitude_x  = $fzone_coordinate['zone_centralzipcode_latitude'];
			$latitude_y  =  $fzone_coordinate['zone_centralzipcode_longitude'];
		}else{
			$postalcode = $pickup;
...
foreach($fzone_coor as $indx=>$fzone_coordinate) {
	...
	if(!empty($_REQUEST)) {
		...
		if($dropoff==$fzone_coordinate['zone_central_zipcode']){
			$longitude_x  = $fzone_coordinate['zone_centralzipcode_latitude'];
			$latitude_y  =  $fzone_coordinate['zone_centralzipcode_longitude'];
		}else{
			$postalcode = $dropoff;
