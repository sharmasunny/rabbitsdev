...
?>
<div class="wrap"> 
	<h2>Large Size Delivery Price List</h2>
	<button type="button" class="btn btn-info btn-lg" data-toggle="modal" data-target="#large-zone-option">Add New Zone Option</button>
	<!-- Modal -->
	<div id="large-zone-option" class="modal fade" role="dialog">
	  <div class="modal-dialog">
	    ...
	    <div class="modal-content">
	      ...
	      <div class="modal-body">
	        <form role="form" action="inc/zone-management/large-option-add.php" method="post" id="largeaddzoneform">
	...
	<?php
	/*
	*	@this query will join record between zone table and large_zone_detail  table
	*/
	...
	$users_records = $conn->query($users_sql);
	?>
	<form name="zone_option_list" method="post" action="inc/zone-management/large-option-update.php?info=delall">	
	<table class="" id="largezoneoptionlist" style="width:100%">
		<thead>
			<tr>
				<th width="10px" style="background:none"><input type="checkbox" id="largeallzoneopt" /></th>
				<th width="14px">ID</th>
				<th>Zone From</th> 
			...
			if ( $users_records->num_rows > 0 ) { ?>
				<script type="text/javascript">
					jQuery(document).ready(function(){
						jQuery('#largezoneoptionlist').dataTable({ 
							"aaSorting": [[ 0, "desc" ]],
						});
						jQuery('#largeallzoneopt').click(function(event) {  //on click
						if(this.checked) { // check select status
						    jQuery('.largezoneoptid').each(function() { //loop through each checkbox
				...
				$price_urgent=$user_record['price_urgent'];
					?>
					<tr>
					<th><input class="largezoneoptid" type="checkbox" name="zoneid[]" id="zoneid" value="<?php echo $zone_opt_id;?>" /></th>
						...
						<td style="text-align: center;">							
							<a onclick="javascript:return confirm('Are you sure, want to delete record ?')" href="inc/zone-management/large-option-update.php?info=del&did=<?php echo $zone_opt_id;?>">
							<img src="images/delete.png" title="Delete" alt="Delete" />
							</a>
							&nbsp;&nbsp;&nbsp;
							<a href="inc/zone-management/large-option-update.php?info=edit&eid=<?php echo $zone_opt_id;?>" class="siframe">
