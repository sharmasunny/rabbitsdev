...
foreach($placemark_arr as $key => $placemark_item) {
	...
	/*
	*	@this query will insert all record from zone table 
	*/
	$sql = "INSERT INTO ".$zones_table." (zone_id,zone_name, zone_coordinates) VALUES (".$jq.",'".$placemark_item['name']."', '".$placemark_item['Polygon']['outerBoundaryIs']['LinearRing']['coordinates']."') ON DUPLICATE KEY UPDATE zone_coordinates = VALUES(zone_coordinates)";
	$conn->query($sql);
	$jq++;
}
if(isset($_GET['success']) && $_GET['success'] == 1)
{
	...
	foreach($placemark_arr as $key => $placemark_item) {
		...
		*	@this query will insert all record from zone table 
		*/
		$sql = "INSERT INTO ".$zones_table." (zone_id,zone_name, zone_coordinates) VALUES (".$iq.",'".$placemark_item['name']."', '".$placemark_item['Polygon']['outerBoundaryIs']['LinearRing']['coordinates']."') ON DUPLICATE KEY UPDATE zone_coordinates = VALUES(zone_coordinates)";
		//echo $sql;die;
		$conn->query($sql);
		$iq++;
	}?>
	...
	<table class="" id="zonelist" style="width:100%">
		<thead>
			<tr>
				...
				<th>Zone Central Postal Code</th>                
